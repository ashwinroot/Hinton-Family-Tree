...
'''
12 hidden units; Part 3 of the question

Does not do the split.. Just one Complete Run with 1000 epochs
...
prediction,person_hidden = model_build(n_person,n_relationship,12)
...
with tf.Session() as sess:
    ...
    plt.legend()
    plt.show()
    fig1.savefig('HintonFamilyTree_3.png')
    ...
    print("Mean train accuracy for first 500 epochs,Standard Deviation: ", np.mean(train_matrix[:500]),
          ...
          np.std(test_matrix[-500:]))
writer = tf.summary.FileWriter("/tmp/a3p1/1")
