...
def model_build(n_dim,n_hidden,n_class):
    ...
    hidden_layer2 = {'weights' : tf.Variable(tf.random_normal([n_hidden,n_hidden])),
                     'bias' : tf.Variable(tf.random_normal([n_hidden]))}
    #output layer params
...
def train(train_x,train_y,test_x,test_y,n_hidden,n_classes,batch_size):
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(0,hm_epochs):
            ...
            train_matrix.append(train_acc / train_x.shape[0])
            test_matrix.append(test_acc /test_x.shape[0])
            error_matrix.append(epoch_loss)
            iter_matrix.append(epoch)
    # fig = plt.figure("Back Prop using Tensorflow")
    # plt.plot(range(0,hm_epochs),train_matrix,':b' ,label= 'train-tensor')
    # plt.plot(range(0, hm_epochs), test_matrix, 'b', label='test-tensor')
    # plt.plot(range(0, hm_epochs), My_imp_train, ':r', label='train-numpy')
    # plt.plot(range(0, hm_epochs), My_imp_test, 'r', label='test-numpy')
    # plt.xlabel("#Epochs")
    # plt.ylabel("Accuracy")
    # plt.legend()
    # fig.show()
    # fig.savefig("BackProp- 5 hidden units")
...
if __name__ == '__main__':
    ...
    for hidden in n_hidden_layer:
        train_matrix = []
        test_matrix = []
        train(train_x,train_y,test_x,test_y,hidden,1,100)
        print("Average Train accuracy :" , np.mean(train_matrix) , "Average Test accuracy :", np.mean(test_matrix))
        tensor_train.append(np.max(train_matrix))
        tensor_test.append(np.max(test_matrix))
    fig1 = plt.figure("Over the hidden layers")
    plt.plot(n_hidden_layer, tensor_train, ':r', label='numpy-train')
    plt.plot(n_hidden_layer, tensor_test, 'r', label='numpy-test')
    plt.plot(n_hidden_layer, numpy_train, ':b', label='tensor-train')
    plt.plot(n_hidden_layer, numpy_test, 'b', label='tensor-test')
    plt.ylabel('Accuracy')
    plt.xlabel('Number of hidden layers')
    plt.title('Accuracy test over epochs: tf vs np')
    plt.legend()
    plt.show()
